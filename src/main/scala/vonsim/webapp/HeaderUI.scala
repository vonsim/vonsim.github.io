...
class HeaderUI(s: VonSimState) extends VonSimUI(s) {
  ...
  val tutorialDropdown = div(
    ...
    div(
      ...
      dropdownItemFactory("whyassembly", "¿Por que Assembly?"),
      dropdownItemFactory("vonsim", "Sobre VonSim"),
      dropdownItemFactory("basic", "Básico"),
      dropdownItemFactory("variables", "Variables"),
      dropdownItemFactory("code", "Registros e instrucciones")
    )
  ).render
